...
2. 从 `images/` 随机挑选一张图片。
3. 通过 `oss-s3/` 上传图片，得到公开 URL，并回写到 `douyin.generated.md`。
4. 打开抖音创作者平台，进入“发布文章”，通过“一键导入”导入成品 Markdown。
5. 将文章标题开头的方括号去掉。例如 `[新的文章主题]不出一直打？？` 会变成 `新的文章主题不出一直打？？`。
6. 将文章摘要设置为 `本作品roll30位体验，打出“666”安排！`。
7. 点击文章头图的“AI生成”，等待抖音生成完成。
8. 添加话题 `#暗区突围`。
9. 打开配乐选择面板，从“推荐”列表前 5 首中随机选择一首。
10. 点击文章页底部的“发布”。
...
首次运行会打开浏览器，请完成抖音登录后回到终端按回车。脚本会进入“发布文章”页，点击“一键导入”并导入 `douyin.generated.md`，然后自动完成上述设置并发布。登录状态保存在 `.douyin-browser/`。
...
只生成 `douyin.generated.md`，不打开浏览器。`--headless` 可用于无界面运行，但首次登录和需要人工处理验证码时不要使用它。
